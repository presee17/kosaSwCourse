...
public class BoardDao {
	...
	public List<Board> selectByPage(int pageNo, int rowsPerPage){
		...
		List<Board> list = jdbcTemplate.query(sql, new Object[] { (pageNo - 1) * rowsPerPage, rowsPerPage },
				new RowMapper<Board>() {
					@Override
	...
	public Board selectByPk(int boardNo){
		...
		Board board = jdbcTemplate.queryForObject(sql, new Object[] { boardNo }, new RowMapper<Board>() {
	...
	public int update(Board board){
		...
		int rows = jdbcTemplate.update(sql, board.getTitle(), board.getContent(), board.getNo());
		return rows;
	}
